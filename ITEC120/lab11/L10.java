...
//}
      ...
      public static int print(int num, int count[])
     {
        ...
        {
        Random randObj = new Random();
        int[] counters = new int[upperBound];
        int number;
        
        for(int i = 0;i < total; ++i) {

            number = randObj.nextInt(upperBound);
            counters[i] = number;

        }
            for (int j = 0; j < counters.length; ++j) {

                System.out.println(this.print(j, counters)); // tried to pass each element of the array to the print method and and only pass the handle and nt sure how to overcome this
            }
