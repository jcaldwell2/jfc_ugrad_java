...
public class JavaWars extends ApplicationAdapter implements InputProcessor {
    ...
    public void create() {
        ...
        for (int i = 0; i < 10; i++) {
            e = new Enemy(MathUtils.random(-250, 250), MathUtils.random(-250, 250));
            enemies.add(e);
...
//                h-10/PIXELS_TO_METERS);
    ...
    private void createEnemyBody(Enemy e, Sprite _e) {


        BodyDef enemyBodyDef = new BodyDef();
        enemyBodyDef.type = BodyDef.BodyType.DynamicBody;
        enemyBodyDef.position.set((e.getXLoc() + _e.getWidth() / 2) /
                        PIXELS_TO_METERS,
                (e.getYLoc() + _e.getHeight() / 2) / PIXELS_TO_METERS);

        enemyBody = world.createBody(enemyBodyDef);

        PolygonShape shape = new PolygonShape();
        shape.setAsBox(_e.getWidth() / 2 / PIXELS_TO_METERS, _e.getHeight()
                / 2 / PIXELS_TO_METERS);
